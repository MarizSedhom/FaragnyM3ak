...
export class MoviePreviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (id) {
        this.movieId = Number.parseInt(id, 10)
        this.loadMovieData()
    ...
    this.movieService
      ...
      .pipe(
        ...
        switchMap((relatedMovies) => {
          ...
          // Now fetch details for these movies to get their durations and descriptions
  ...
  convertToBasicMovieFormat(relatedMovie: RelatedMovie): Movie {
    return {
      ...
      rating: relatedMovie.rating || 0, // Use the rating from relatedMovie if available
      ratingCount: relatedMovie.ratingCount || 0, // Use the ratingCount from relatedMovie if available
  ...
  convertToMovieFormat(relatedMovie: RelatedMovie): Movie {
    ...
    if (movieWithDetails) {
      return {
        ...movieWithDetails,
        // Only use the match percentage as a fallback if no description exists
        description: movieWithDetails.description || `${relatedMovie.matchPercentage}% Match to ${this.movie?.title}`,
      }
    }
